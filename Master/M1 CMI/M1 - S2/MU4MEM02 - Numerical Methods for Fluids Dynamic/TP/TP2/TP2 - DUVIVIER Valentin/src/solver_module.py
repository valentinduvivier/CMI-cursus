...
def upwind(m, Ux):
   ...
   Int = (Ux<0) * sp.diags([1.], [1], (m-1, m)).toarray() \
       + (Ux>0) * sp.diags([1.], [0], (m-1, m)).toarray()
...
def quick(m, Ux):
   ...
   # quick interpolation
   Int = (Ux>0) * (1/8) * sp.diags([6., 3., -1.], [0, 1, -1], (m-1, m)).toarray() + (Ux<0) * (1/8) * sp.diags([3., 6., -1.], [0, 1, 2], (m-1, m)).toarray()
