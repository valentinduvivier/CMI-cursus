...
def assembl_external_forces(forces, model):
    ...
    for i, e in enumerate(forces.el_list_vol):
        fx_el = forces.fx_ev[i]
        fy_el = forces.fy_ev[i]
        cz_el = forces.cz_ev[i]
        
        dofe = e.get_dof()
        
        F[dofe] += e.elementary_volume_forces([fx_el, fy_el, cz_el])
...
def stresses(U, model, E, nu):
    ...
    for (i,e) in enumerate(mesh.elem_list):
        Sig[nS*i:nS*(i+1)] = e.stresses(U[e.get_dof()], e.mat, e.sect, E, nu)
    return Sig
