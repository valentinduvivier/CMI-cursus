...
class SolidT6(Triangle6):
    ...
    def shape_functions(self, xi, eta):
        ...
        N = np.array([xi*(2*xi-1), eta*(2*eta-1), (1-xi-eta)*(1-2*xi-2*eta), 4*xi*eta, 4*eta*(1-xi-eta), 4*xi*(1-xi-eta)])
        DN = np.array([[4*xi-1, 0, -3+4*xi+4*eta, 4*eta, -4*eta, 4-8*xi-4*eta],
                       [0, 4*eta-1, -3+4*xi+4*eta, 4*xi, 4-4*xi-8*eta, -4*xi]])
        return N, DN
    ...
    def elementary_stiffness(self, mat, sect=None):     
        ...
        Ke = np.zeros((self.el_dof,self.el_dof)) #self.el_dof=12 here
        for i in range(self.ngauss):
            # combination of stiffness matrices of three gauss point
            Ke += self.wg[i] * self.detJ*np.dot(self.Be[i].T,np.dot(mat.C,self.Be[i]))
    ...
    def elementary_distributed_forces(self, el_force):
        ...
        for i in range(self.ngauss):
            ...
            fe   += self.wg[i] * self.detJ*np.kron(N,np.array([fx,fy]))
    ...
    def elementary_volume_forces(self, el_force):
        ...
        ----------
        el_force = [fx,fy,cz] : array,list
            contains volume forces :math:`(f_x,f_y)`
    ...
    def stresses(self, Ue, mat, sect, E, nu):
        ...
        for i in range(self.ngauss):
            sig_g[[4*i, 4*i+1, 4*i+3]] = self.wg[i] * np.dot(C, np.dot(self.Be[i], Ue))
            sig_g[4*i+2] = lamb*np.trace(self.Be[i])
