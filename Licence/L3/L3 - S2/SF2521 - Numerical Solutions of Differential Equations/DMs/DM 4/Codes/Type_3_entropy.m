%% 2.2 - Tests with non-horizontal bottom - Type 3 - entropy term
...
% ----------------------------------------------------------------------------------------------- %
    ...
    % IC :
        ...
        % Moment
            for k = 2:N_x+1
                u(2,k) = 2.1;    % Subcritical   --> u < sqrt(gh) --> m < h*sqrt(gh)
            end
            
        % Dirichlet condition for inflow
            Inflow_Condition = 2.1;
        ...
        for t = 1:N_t            
            ...
            % BC
                % Height
                    u(1,1)      = 1;    % Dirichlet BC for ghost cell  (j =  -1) 
                    u(1,N_x+2)  = 1;    % Dirichlet BC for ghost cell  (j = N+2)
...
%% Presentation of results
    ...
    figure(93)
        k = N_t;
        p_93(1:3) = plot(x, B, 'r--', x, U_2(2*k + 1, 2:end-1) + B, 'b--', x, U_2(2*k + 2,2:end-1)./U_2(2*k + 1,2:end-1), 'g-');
        hold on;
        p_93(4) = plot([0,10], [3.1,3.1], 'r-');
        axis([0 10 -0.2 4]);
        xlabel('Position on the mesh [m]'); title(['t = ',num2str(round(k*T/N_t,2)),' s']);
        grid on;
        legend([p_93], {'B', 'h+B', 'u', 'Sonic speed'}, 'location', 'northwest');
        hold off;
