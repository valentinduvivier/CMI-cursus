%% 2.2 - Tests with non-horizontal bottom - Type 1
    ...
    % General variables
        ...
        N_x = 120;      % Number of point defining the mesh
        N_t = 1000;     % Number of iteration
        ...
        for t = 1:N_t            
            ...
            % BC
                % Height
                    ...
                    u(1,N_x+2)  = 0.9;                  % Dirichlet BC for ghost cell   (j = N+2)
...
%% Presentation of results
    ...
    figure(65)
        k = N_t;
        p_65 = plot(x, B, 'r--', x, U_2(2*k + 1, 2:end-1)+B, 'b--', x, U_2(2*k + 2,2:end-1)./U_2(2*k + 1,2:end-1), 'g-');
        axis([0 10 -0.2 2]);
        xlabel('Position on the mesh [m]'); title(['t = ',num2str(round(k*T/N_t,2)),' s']);
        grid on;
        legend([p_65], {'B', 'h+B', 'u'}, 'location', 'northwest');
