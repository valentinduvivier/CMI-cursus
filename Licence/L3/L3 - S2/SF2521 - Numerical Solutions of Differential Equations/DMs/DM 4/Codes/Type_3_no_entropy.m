%% 2.2 - Tests with non-horizontal bottom - Type 3 - no entropy term
...
%% Presentation of results
    ...
    figure(83)
        k = N_t;
        p_83 = plot(x, B, 'r--', x, U_2(2*k + 1, 2:end-1) + B, 'b--', x, U_2(2*k + 2,2:end-1)./U_2(2*k + 1,2:end-1), 'g*-');
        axis([0 10 -0.2 5.5]);
        xlabel('Position on the mesh [m]'); title(['t = ',num2str(round(k*T/N_t,2)),' s']);
        grid on;
        legend([p_83], {'B', 'h+B', 'u'}, 'location', 'northwest');
