% Homework Assignement 2 - Hyperbolic Equations
    ...
    % course and try to hhave a better understanding on how to deal with
...
% Q 2.1 - 1 : resolution system
    ...
    % We here consider : 
        %   - alpha = Delta_x/Delta_t
            ...
            iter_t = 300;           % Number of iteration - define the size of the loop that gives U
                                    % We ensure to have iter_t = 3*50 if we
                                    % want to iterate over one revolution
                                    % (condition : keeping Delta_t = Delta_x/10);

            x = linspace(0, L, n);  % Vector x to be used for initial wave's height
            Delta_x = L/((n-1));    % Space-step
        
            Delta_t = Delta_x/10;  % Time-step such that we ensure at the maximum stability - Delta_t/Delta_x < 1
                    
        % We build the vector tn : vector of size of number of iterations
            for k = 1:iter_t
                t_n(k) = k*Delta_t;
            end

            alpha = Delta_x/Delta_t;    % Parameter for discrete equation
            
            if alpha > Delta_x/Delta_t
                disp('error');
            end
...
%% Display of the wave
        ...
        figure(3)
            subplot(2,2,1)
            plot(x, U(2*1-1,2:end-1), 'r');
            axis([0 10 1 1.1]);
            xlabel('position on the mesh [0;L](m)'); ylabel('Wave height (m)'); title('Wave at t = Delta_t s');

            subplot(2,2,2)
            plot(x, U(2*50-1,2:end-1), 'r');
            axis([0 10 1 1.1]);
            xlabel('position on the mesh [0;L](m)'); ylabel('Wave height (m)'); title('Wave at t = 50*Delta_t s = 1 bouncing');

            subplot(2,2,3)
            plot(x, U(2*100-1,2:end-1), 'r');
            axis([0 10 1 1.1]);
            xlabel('position on the mesh [0;L](m)'); ylabel('Wave height (m)'); title('Wave at t = 100*Delta_t s = 2 bouncings');

            subplot(2,2,4)
            plot(x, U(2*150-1,2:end-1), 'r');
            axis([0 10 1 1.1]);
            xlabel('position on the mesh [0;L](m)'); ylabel('Wave height (m)'); title('Wave at t = 150*Delta_t s = 3 bouncings');
